...
const reducer = (state = initaialstate, actions) => {
  ...
  if (actions.type === TYPE.addToWishlist) {
    const find = FindItem(state.wishlist, actions.payload.id);

    if (find) {
      return {
        ...state,
        wishlist: state.wishlist.filter((el) => el.id !== find.id),
      };
    } else {
      return {
        ...state,
  ...
  if (actions.type === "REMOVE_FROM_WISHLIST") {
    return {
      ...state,
      wishlist: state.wishlist.filter((item) => item.id !== actions.payload),
    };
  }
